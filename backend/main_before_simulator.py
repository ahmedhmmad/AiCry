    ...
    try:
        ...
        return {
            ...
            "individual_recommendations": {
                "technical": {"recommendation": tech_rec, "confidence": tech_conf},
                "simple_ai": {"recommendation": simple_rec, "confidence": simple_conf},
                "advanced_ai": {"recommendation": advanced_rec, "confidence": advanced_conf}
            }
        }
...
@app.post("/backtest/improved/{symbol}")
async def run_improved_backtest(
    symbol: str,
    days: int = Query(default=30, ge=14, le=90, description="عدد أيام الاختبار"),
    interval: str = Query(default="1h", description="الفترة الزمنية")
):
    """
    تشغيل اختبار أداء محسن مع دقة أفضل
    """
    try:
        from backtesting import improved_backtesting_engine
        result = improved_backtesting_engine.run_improved_backtest(symbol, days, interval)
        return result
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))

@app.get("/ai/feature-report/{symbol}")
async def get_feature_report(symbol: str):
    """
    الحصول على تقرير مفصل عن الميزات المُستخدمة
    """
    try:
        klines_data = binance_client.get_klines(symbol, "1h", 500)
        if not klines_data:
            raise HTTPException(status_code=404, detail=f"Could not fetch data for {symbol}")
        ...
        # تجنب استخدام advanced_ai إذا كان يسبب مشاكل
        try:
            report = advanced_ai.generate_feature_report(close_prices, volumes)
        except Exception as e:
            # تقرير بسيط في حالة الخطأ
            report = {
                "total_features_generated": 30,
                "selected_features_count": 25,
                "top_20_features": ["rsi", "macd", "bb_position", "ma_ratio_5_20", "volatility"],
                "feature_categories": {
                    "trend_indicators": ["ma_5", "ma_20", "ema_21"],
                    "momentum_indicators": ["rsi", "stoch_k", "momentum_5"],
                    "volatility_indicators": ["bb_position", "volatility"],
                    "other_features": ["volume_change", "price_change"]
                },
                "enhancement_status": "basic_indicators_only",
                "error_details": str(e)
            }
