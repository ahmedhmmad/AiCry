...
from enhanced_indicators import calculate_enhanced_indicators, select_best_features, create_feature_importance_report
...
def engineer_advanced_features(self, prices: List[float], volumes: List[float] = None) -> pd.DataFrame:
    ...
    """
    try:
        # استخدام المؤشرات المحسنة
        features_df = calculate_enhanced_indicators(prices, volumes)
        
        print(f"تم إنشاء {features_df.shape[1]} ميزة متقدمة")
    ...
    def engineer_advanced_features_old(self, prices: List[float], volumes: List[float] = None) -> pd.DataFrame:
        """
        هندسة ميزات متقدمة للتحليل
        """
...
def select_important_features(self, df: pd.DataFrame) -> List[str]:
    """
    اختيار الميزات المهمة باستخدام طرق إحصائية متقدمة
    """
    try:
        # إنشاء هدف وهمي للاختبار
        target = df['close'].shift(-1) - df['close']  # تغيير السعر
        
        # استخدام خوارزمية اختيار الميزات المتقدمة
        selected_features = select_best_features(df, target, max_features=60)
        
        print(f"تم اختيار {len(selected_features)} ميزة من أصل {df.shape[1]}")
        
        return selected_features
    except Exception as e:
        print(f"خطأ في اختيار الميزات: {e}")
        # العودة للطريقة القديمة
        exclude_cols = ['close', 'open', 'high', 'low', 'hour', 'day_of_week']
        feature_cols = [col for col in df.columns if col not in exclude_cols]
        return feature_cols[:50]
    def select_important_features_old(self, df: pd.DataFrame) -> List[str]:
        """
        اختيار الميزات المهمة للتدريب
        """
        # استبعاد الأعمدة غير المناسبة للتدريب
...
def generate_feature_report(self, prices: List[float], volumes: List[float] = None) -> Dict[str, Any]:
    ...
    try:
        features_df = self.engineer_advanced_features(prices, volumes)
        target = features_df['close'].shift(-1) - features_df['close']
        
        report = create_feature_importance_report(features_df, target)
